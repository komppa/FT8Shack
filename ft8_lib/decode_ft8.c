...
// Get audio buffer variable from wave.c file to test it
extern struct audioBuffer buffer;
extern struct wasm_messages msgs;
...
EMSCRIPTEN_KEEPALIVE
#endif
struct wasm_message* get_msg_by_index(int message_index) {
    // TODO check if msgs array have so many messages that
    // is requested.
    return &msgs.messages[message_index];
}
...
EMSCRIPTEN_KEEPALIVE
#endif
uint16_t* get_score_by_message_id(int message_index) {
    return &msgs.messages[message_index].score;
}


/**
 * @brief Get time_sec by two ints (read two ints)
 * 
 */
#ifdef EMSCRIPTEN
EMSCRIPTEN_KEEPALIVE
#endif
uint16_t* get_time_by_message_id(int message_index) {
    return &msgs.messages[message_index].time_sec[0];
}
...
EMSCRIPTEN_KEEPALIVE
#endif
uint16_t* get_freq_by_message_id(int message_index) {
    return &msgs.messages[message_index].freq_hz[0];
}
...
EMSCRIPTEN_KEEPALIVE
#endif
char* get_text_by_message_id(int message_index) {
    return &msgs.messages[message_index].text[0];
}
...
int main() {    // int argc, char *argv[]
    ...
    // buffer.audio_p[0] = 21065;
    // buffer.audio_p[1] = 17990;
    // buffer.audio_p[2] = 0x027f;
    // buffer.audio_p[3] = 0x0500;
    // buffer.audio_p[4] = 0x5741;
    // buffer.audio_p[5] = 0x5645;
    // buffer.audio_p[6] = 0x666d;
    // buffer.audio_p[7] = 0x7420;

    // buffer.audio_p[8] = 0x1000;
    // buffer.audio_p[9] = 0x0000;
    // buffer.audio_p[10] = 0x0100;
    // buffer.audio_p[11] = 0x0100;
    // buffer.audio_p[12] = 0xe02e;
    // buffer.audio_p[13] = 0x0000;
    // buffer.audio_p[14] = 0xc05d;
    // buffer.audio_p[15] = 0x0000;

    // buffer.audio_p[16] = 0x0200;
    // buffer.audio_p[17] = 0x1000;
    // buffer.audio_p[18] = 0x6461;
    // buffer.audio_p[19] = 0x7461;
    // buffer.audio_p[20] = 0x4073;
    // buffer.audio_p[21] = 0x0500;
    // buffer.audio_p[22] = 0x32ee;
    // buffer.audio_p[23] = 0x42f1;
    
    /*
    buffer.audio_p[24] = 0x7420;
    buffer.audio_p[25] = 0x7420;
    buffer.audio_p[26] = 0x7420;
    buffer.audio_p[27] = 0x7420;
    buffer.audio_p[28] = 0x7420;
    buffer.audio_p[29] = 0x7420;
    buffer.audio_p[30] = 0x7420;
    buffer.audio_p[31] = 0x7420;
    */
    ...
    // "Initialize" msgs count
    msgs.num_decoded = 0;

    // Accepted arguments
    const char* wav_path = NULL;    // This is not needed since we have custom fread function
    bool is_ft8 = true;

    // Parse arguments one by one
    int arg_idx = 1;
    // Disabled argument check since those are injected straight to memoty
    /*
    while (arg_idx < argc)
    {
    ...
    */
    ...
    num_decoded = 0;    // Int declaration was here, but was moved to be global varaible
    ...
    for (int idx = 0; idx < num_candidates; ++idx)
    {
        ...
        if (found_empty_slot)
        {
            ...
            // output
            // TODO init msgs couint to be 0!
            // printf("000000 %3d %+4.2f %4.0f ~  %s\n", cand->score, time_sec, freq_hz, message.text);


            // Save decoded message
            msgs.messages[msgs.num_decoded].score = cand->score;

            // Time sec has been divided to two separate variables (in a same array)
            // the first one is for integer part and the second index is for 2-digit decimal.
            msgs.messages[msgs.num_decoded].time_sec[0] = (int)time_sec;
            msgs.messages[msgs.num_decoded].time_sec[1] = (int)((time_sec - (int)time_sec) * 100);

            msgs.messages[msgs.num_decoded].freq_hz[0] = (int)freq_hz;
            msgs.messages[msgs.num_decoded].freq_hz[1] = (int)((freq_hz - (int)freq_hz) * 100);
            
            strcpy(
                msgs.messages[msgs.num_decoded].text,
                message.text
            );
            

            // Tell that there is one more message decoded
            msgs.num_decoded++;

            // int16_t  cand->score     dB
            // float    time_sec        when started?
            // float    freq_hz         frequency
            // char[25] message.text    plain text
