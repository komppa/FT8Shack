...
#ifdef EMSCRIPTEN
// Global variable for storing message to be generated as audio
EMSCRIPTEN_KEEPALIVE
char message_in[13];

/**
 * @brief Get message that has been passed in
 */
EMSCRIPTEN_KEEPALIVE
char* get_message_in() {
    return &message_in[0];
...
int main() {
    ...
    */
    ...
    #ifdef EMSCRIPTEN
    const char* message = &message_in[0];
    // const char* message = "CQ OH9BD KP26";
    #else
    const char* message = &msg[0];  //argv[1];
    #endif

    // printf("MESSAGE='%s'\n", message);

    const char* wav_path = "myMessageFromC.wav";   // argv[2];
